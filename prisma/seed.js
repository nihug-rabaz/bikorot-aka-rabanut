...
async function main() {
  ...
  const categories = [
    { name: 'הלכה', order: 2, criteria: ['נספח הלכתי', 'התקני שבת', 'ערכת הבדלה', 'בית כנסת + עזרת נשים', 'ספרי תורה + מספר צבאי', 'עירוב', 'חגים', 'צומות', 'תפילות', 'מזוזות', { label: 'ליווי מתגיירים', isActive: false }] },
    { name: 'כשרות', order: 3, criteria: ['רכש חוץ', 'מערכת מכ"ם', 'הדרכת מכ"שים', 'שילוט', 'בקיאות טבחים ומכ"שים', 'סימונים', 'חירורים', 'אפיון כלים', 'טבילת כלים', 'דגים ובשר', 'אזרוח כשרות', 'פיקוח עבודות ובקרת כשרות', 'תשתיות מטבח', 'ברירה וניפוי', 'תבלינים', 'ציוד אירועים', 'התנהלות שבת', 'פיקוח שבת', 'קונדיטוריה', 'כוורת', 'מהדרין', 'בישול ישראל', 'הפרשת חלה', 'חדר מכ"ש', 'מטבחונים'] },
        ...
        criteria: {
          create: cat.criteria.map((c, index) =>
            typeof c === 'string'
              ...
              : { label: c.label, type: c.type ?? 'RADIO', order: index + 1, isActive: c.isActive ?? true }
          )
        }
      }
    })
